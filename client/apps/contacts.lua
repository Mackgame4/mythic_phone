...
RegisterNUICallback( 'EditContact', function( data, cb )
    ...
    TriggerServerEvent('mythic_phone:server:EditContact', securityToken, 'EditContact', data.id, data.name, data.number)
end)

RegisterNUICallback( 'DeleteContact', function( data, cb )
    actionCb['DeleteContact'] = cb
    TriggerServerEvent('mythic_phone:server:DeleteContact', securityToken, 'DeleteContact', data.id)
end)